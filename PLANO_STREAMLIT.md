...
Implementar o dashboard "Análise de Maturidade em Processos" como um app **Streamlit**, usando o formulário Excel como fonte de dados.

---

## 1. Objetivo e escopo

- Ler `formulario\F.O.091.GOCO - Analise de Maturidade em Processos.xlsx` (abas `Avaliação`, `Indicadores`, `Treinamento`, `Qualidade´).
- Implementar em Python/pandas as transformações de dados e cálculos de pontuação.
- Entregar 6 páginas: **Visão Geral, Documentação, Indicadores, Treinamento, Evolução, Plano de Ação**.
...
Implementar as 3 funções de conversão e as 3 tabelas fato:
...
### Fase 6 — Apresentação
- Executar local: `streamlit run app.py` (opcional `--server.headless true`).
